...
function delegate() {
    ...
    VOTING_POWER_PRE=$(checkVotingPower)
    VOTING_POWER_POST=0
    ...
    while (($ITERATION < $MAX_ITERATIONS)) && (($VOTING_POWER_POST <= $VOTING_POWER_PRE)); do
        VOTING_POWER_POST=$(checkVotingPower)
        sleep 2
        ITERATION=$((ITERATION+1))
    done

    if (($ITERATION == $MAX_ITERATIONS)); then
        echo "Could not confirm Voting Power update on consumer chain after 60 seconds."
    else
...
function jailConsumer() {
    ...
    vagrant scp files/generated/provider_priv_validator_key2.json consumer-chain-validator1:$CONSUMER_HOME/config/priv_validator_key.json

    echo "Restaring $CONSUMER_APP..."
    vagrant ssh consumer-chain-validator1 -- "$CONSUMER_APP --home $CONSUMER_HOME start --log_level trace --pruning nothing --rpc.laddr tcp://0.0.0.0:26657 --grpc.address 0.0.0.0:9090> /var/log/consumer.log 2>&1 &"
...
function jailProvider() {
    ...
    echo "Copying duplicate private validator key to validator..."
    vagrant scp files/generated/provider_priv_validator_key3.json provider-chain-validator1:$PROVIDER_HOME/config/priv_validator_key.json

    echo "Restaring $PROVIDER_APP..."
    ...
    MAX_ITERATIONS=30
    ITERATION=0
    VOTING_POWER=$(checkVotingPower "provider-chain-validator1")
    while (($ITERATION < $MAX_ITERATIONS)) && (($VOTING_POWER != 0)); do
        VOTING_POWER=$(checkVotingPower "provider-chain-validator1")
        ITERATION=$((ITERATION+1))
        sleep 2

...
function jailProvider() {
    ...
    echo "Copying duplicate private validator key to validator..."
    vagrant scp files/generated/provider_priv_validator_key2.json provider-chain-validator1:$PROVIDER_HOME/config/priv_validator_key.json
    ...
    if (($VOTING_POWER_PROVIDER == 0)); then
        echo "Validator has been jailed on provider chain."
    else
        echo "Could not confirm that validator has been jailed within 60 seconds on provider chain."
    fi
    ...
    if (($VOTING_POWER_CONSUMER == 0)); then
        echo "Validator has been jailed on consumer chain."
    else
        echo "Could not confirm that validator has been jailed within 60 seconds on consumer chain."
    fi

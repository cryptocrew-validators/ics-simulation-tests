...
function prepareRelayer() {
  ...
  vagrant ssh provider-chain-validator1 -- "echo $RELAYER_MNEMONIC > .mn && $HERMES_BIN --config $HERMES_CONFIG keys add --chain provider-chain --mnemonic-file .mn || true && $HERMES_BIN --config $HERMES_CONFIG keys add --chain consumer-chain-1 --mnemonic-file .mn || true"
}
...
function testConnection() {
  ...
  CONNECTION=$(vagrant ssh provider-chain-validator1 -- "$HERMES_BIN query connections --chain consumer-chain-1")
...
function testChannel() {
  ...
  CHANNEL=$(vagrant ssh provider-chain-validator1 -- "$HERMES_BIN query channels --chain consumer-chain-1")

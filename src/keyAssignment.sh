...
function validateAssignedKey() {
  ...
  sleep 1
  vagrant ssh consumer-chain-validator1 -- "$CONSUMER_APP --home $CONSUMER_HOME start --log_level $CHAIN_LOG_LEVEL --pruning nothing --rpc.laddr tcp://0.0.0.0:26657 --grpc.address 0.0.0.0:9090> /var/log/chain.log 2>&1 &"
  echo "Restarted consumer-chain-validator1."

  echo "Validating key assignment consumer-chain-validator1: $1"

  UPDATED_PUBKEY_VALUE=$(cat files/generated/priv_validator_key1_UPDATED_"$1".json | jq -r '.pub_key.value')
